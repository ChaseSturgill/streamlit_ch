...
if (st.button('Calculate')):
    ...
    try:
        test_response = requests.get("https://chapi.cloudhealthtech.com/olap_reports/cost/history?&api_version=2&client_api_id=" + client_id + "&dimensions[]=time&dimensions[]=AWS-Service-Category&measure[]=cost&interval=monthly", headers=my_headers)
    ...
    ################## EC2 On-Demand NFUs ##########################
    ec2_response = requests.get("https://chapi.cloudhealthtech.com/olap_reports/usage/instance?&api_version=2&query=is_active=1&client_api_id=" + client_id + "&dimensions[]=time&dimensions[]=AWS-Coverage-Type&measures[]=nf_instances&interval=monthly&filters[]=time:select:-2, -3, -4", headers=my_headers)
    ...
    #Initalize labels for data 
    ec2labels = [threeMonths.strftime("%m-%Y"), twoMonths.strftime("%m-%Y"), lastMonth.strftime("%m-%Y")]
    ...
    odpdf['Monthly % Change'] = (round(((odpdf[lastMonth.strftime("%m-%Y")] - odpdf[twoMonths.strftime("%m-%Y")])/odpdf[twoMonths.strftime("%m-%Y")])*100, 2))

    #Create column with % change from previous month and the month before
    ec2df['Monthly % Change'] = (round(((ec2df[lastMonth.strftime("%m-%Y")] - ec2df[twoMonths.strftime("%m-%Y")])/ec2df[twoMonths.strftime("%m-%Y")])*100, 2))
    ...
    #create column with % change from previous month and the month before
    s3df_all['Monthly % Change'] = ((s3df_all[lastMonth.strftime("%m-%Y")] - s3df_all[twoMonths.strftime("%m-%Y")])/s3df_all[twoMonths.strftime("%m-%Y")])*100
